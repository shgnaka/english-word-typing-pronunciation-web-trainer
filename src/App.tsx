  ...
  return (
    <div className="app-shell" data-testid="app-shell">
      <div className="app-frame">
        <nav className="tabs-panel" aria-label="Primary" data-testid="app-tabs">
          <div className="tabs">
            <button type="button" className={trainer.screen === "practice" ? "active" : ""} data-testid="tab-practice" onClick={() => trainer.setScreen("practice")}>
              {t(language, "tabs.practice")}
            </button>
            <button type="button" className={trainer.screen === "words" ? "active" : ""} data-testid="tab-words" onClick={() => trainer.setScreen("words")}>
              {t(language, "tabs.words")}
            </button>
            <button type="button" className={trainer.screen === "settings" ? "active" : ""} data-testid="tab-settings" onClick={() => trainer.setScreen("settings")}>
              {t(language, "tabs.settings")}
            </button>
            <button type="button" className={trainer.screen === "results" ? "active" : ""} data-testid="tab-results" onClick={() => trainer.setScreen("results")}>
              {t(language, "tabs.results")}
            </button>
          </div>
        </nav>

        <main className="grid app-main">
          <section
            ref={primaryPanelRef}
